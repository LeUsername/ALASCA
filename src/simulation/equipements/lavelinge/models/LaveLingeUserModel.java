...
@ModelExternalEvents(exported = {StartAtEvent.class, EcoLavageEvent.class, PremiumLavageEvent.class})
public class LaveLingeUserModel extends AtomicES_Model {
	...
	public static final String URI = "LaveLingeUserModel";
	/**
	...
	public LaveLingeUserModel(String uri, TimeUnit simulatedTimeUnit, SimulatorI simulationEngine) throws Exception {
	...
	public void initialiseState(Time initialTime) {
		this.initialDelay = LaveLingeUserBehaviour.INITIAL_DELAY;
		this.interdayDelay = LaveLingeUserBehaviour.INTER_DAY_DELAY;
		...
		) throws Exception
	{
		...
		vname = this.getURI() + ":" + LaveLingeUserModel.ACTION + ":"+ PlotterDescription.PLOTTING_PARAM_NAME ;
		PlotterDescription pdTemperature = (PlotterDescription) simParams.get(vname) ;
		this.plotter = new XYPlotter(pdTemperature) ;
		this.plotter.createSeries(LaveLingeUserModel.ACTION) ;
	}
	...
	public void userDefinedInternalTransition(Duration elapsedTime) {
		...
		if (this.nextEvent.equals(StartAtEvent.class) ) {
			...
			if(r.nextFloat() > 0.75) {
				...
				this.scheduleEvent(new PremiumLavageEvent(t));
				if (this.plotter != null) {
				...
				Time t = this.getCurrentStateTime().add(d);
				this.scheduleEvent(new EcoLavageEvent(t));
				if (this.plotter != null) {
					this.plotter.addData(
							ACTION,
							this.getCurrentStateTime().getSimulatedTime(),
