...
import simulation2.events.controleur.StartEngineGenerator;
import simulation2.events.controleur.StopEngineGenerator;
import simulation2.events.groupeelectrogene.ProductionEvent;
import simulation2.tools.controleur.Decision;
import simulation2.tools.groupeelectrogene.GroupeElectrogeneState;
...
public class ControllerModel extends AtomicModel {
	...
	protected GroupeElectrogeneState EGState;
...
//		this.setLogger(new StandardLogger());
	...
	public void			initialiseState(Time initialTime)
	{
		...
		this.EGState = GroupeElectrogeneState.OFF;
...
//		}
	...
	public void			userDefinedExternalTransition(Duration elapsedTime)
	{
		...
		for (int i = 0 ; i < current.size() ; i++) {
			if (current.get(i) instanceof ProductionEvent) {
				this.production =
						((ProductionEvent.Reading)
							((ProductionEvent)current.get(i)).
											getEventInformation()).value ;
				this.logMessage("userDefinedExternalTransition|"
						+ this.getCurrentStateTime()
						+ "|EG production = " + this.production) ;
			}
			if (current.get(i) instanceof ConsommationEvent) {
				this.consommation =
						((ConsommationEvent.Reading)
							((ConsommationEvent)current.get(i)).
											getEventInformation()).value ;
...
//		GroupeElectrogeneState oldState = this.EGState ;
		if (this.EGState == GroupeElectrogeneState.ON) {
...
//				 				+ "|new state = " + this.EGState) ;
			...
			assert	this.EGState == GroupeElectrogeneState.OFF;
			if (this.production <= this.consommation) {
				// on l'eteint
				this.triggeredDecision = Decision.START_ENGINE;
				this.EGState = GroupeElectrogeneState.ON ;
